...
from src.filter import filter_and_rank, filter_and_rank_with_rest
...
def run_pipeline(config_path: str = "config.yaml", issue_number: int = 1,
    ...
    print(f"📊 Top picks: {len(filtered)} items | Also found: {len(rest)} more\n")
    ...
    md_output = generate_markdown(filtered, issue_number=issue_number, date=date_str, rest=rest)
    html_output = generate_html(filtered, issue_number=issue_number, date=date_str, rest=rest)
